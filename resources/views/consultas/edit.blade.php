    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('updateConsulta', $rdbmsqrys['id']) }}">
                        @method('PUT')
                        @csrf
                        <input type="hidden" name="rdbms_id" value="{{ $rdbmsqrys['rdbms_id'] }}" />
                        <?php
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                ...
                                <select id="application_id" class="form-control{{ $errors->has('application_id') ? ' is-invalid' : '' }}" name="application_id" required>
                                    ...
                                    @foreach($applications as $row)
                                        @if($rdbmsqrys['application_id'] == $row['id'])
                                            <option selected value="{{ $row['id'] }}">{{ utf8_encode($row['name']) }}</option>
                                        @else
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="select_in" class="form-control{{ $errors->has('select_in') ? ' is-invalid' : '' }}" name="select_in" required>
                                    <option value="">Seleccione...</option>
                                    @foreach($optionsTQ as $key => $value)
                                        @if($key == $rdbmsqrys["select_in"])
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                ...
                                <textarea id="qry_read" type="text" class="form-control{{ $errors->has('qry_read') ? ' is-invalid' : '' }}" name="qry_read" required autofocus>{{ $rdbmsqrys['qry_read'] }}</textarea>
...
<script>
    $(document).ready(function() {
        ...
        $('#regresar').on('click', function(){
            var url = "{{ URL::to('/conexiones/consultas/lista') }}/{{ $rdbmsqrys['rdbms_id'] }}";
