        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="post" action="{{route('relacionpm.update', $id)}}">
                        @method('PUT')
                        @csrf
                                ...
                                @foreach($modulos as $registro)
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        @php
                                            $checked = '';
                                        @endphp
                                        @if(isset($dataUpdate[$registro->id]))
                                            @php
                                                $checked = 'checked';
                                            @endphp
                                        @endif
                                        <input type="checkbox" {{ $checked }} class="form-control{{ $errors->has('execution') ? ' is-invalid' : '' }}" {{ $checked }} value="{{ $registro->id }}" name="{{ $registro->modulename }}" autofocus>
