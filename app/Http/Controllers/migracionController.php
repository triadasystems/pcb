...
class migracionController extends Controller
{
    ...
    public function ejecutarMigracion(Request $request)
    {
        ...
        if ($data["ejecutables2"] == 0)
        {
            $excelTel = $this->ejecutable_externos($consecutivo,$a);
            if ($excelTel === true) 
            {
                if (@is_array($data["status"]["errores"])) {
                    $data["status"]["errores"]=$this->recorre_ok_error($data["status"]["errores"], $this->errores);
        ...
        if ($data["recorrido"] == 0 && $data["ejecutables"] == 1 && $data["ejecutablesad"] == 1 && $data["ejecutables2"] == 1) {
            $cant = $data["cant"];
            $total = $data["total"];
        ...
        if($data["limpia"] == 0 && $data["recorrido"] == 1 && $data["zero_bueno"] == 1 && $data["pasa"] == 1 && $data["ejecutables"] == 1)
        {
            ...
            DB::table('logbook_movements')->insert([
                [
                    ...
                    'description' => 'Generación de '.$tipoEjecución,
                    'tipo' => 'alta',
    ...
    public function ejecutable() {
        //exec("D:\Lectura_ActiveDirectory\ADTelevisa.exe 1 ".$this->passDB." ".$this->passLDAP);
        exec("D:\\Active_Directory_PBC\\ADTelevisa.exe 1 ".$this->passDB." ".$this->passLDAP);
    }

    public function ejecutable2($cons, $operacion)
    {
        $cmd = "D:\\Lectura_Excel\\ExcelesTelevisa.exe ".intval($cons)." $operacion ".$this->passDB;
        //print_r($cmd);
        exec($cmd);

        
        $dir="Trabajando";
        if($this->buscarTxt($dir,$cons) === false) {
    ...
    {
        $cmd = "D:\\Active_Directory_PBC\\ADTelevisa.exe 2 ".intval($cons). " $operacion ".$this->passDB." ".$this->passLDAP;
        //print_r($cmd);
        exec($cmd);
    ...
    public function ejecutable_externos($cons, $operacion)
    {
        $cmd = "D:\\ExcelTelevisa2\\ExcelTelevisa2.exe ".intval($cons)." $operacion ".$this->passDB;
        
        exec($cmd);
        // $dir="Trabajando2";
        $dir="Procesados2";
        if($this->buscarTxt($dir, $cons, 'externos') === false) {
            array_push($this->errores, array("Excel Externos" => "Falla en Excel"));
            
            return true;
        } else {
            //array_push($this->ok_conec, "ExcelesTelevisa");
            array_push($this->ok_conec, array("Excel Externos" => "OK"));
    ...
    public function buscarTxt($dir, $cons, $tipo = null) {
        $pathText = "D:\\ProyectosTF33\\ExpressTelevisa\\".$dir;
